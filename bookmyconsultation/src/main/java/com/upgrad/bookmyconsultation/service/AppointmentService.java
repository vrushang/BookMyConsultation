...
public class AppointmentService {
	...
	public String appointment(Appointment appointment) throws SlotUnavailableException,InvalidInputException{
		...
		Appointment checkAppoExistOrNot = appointmentRepository.findByDoctorIdAndTimeSlotAndAppointmentDate(appointment.getDoctorId(),appointment.getTimeSlot(),appointment.getAppointmentDate());
		if (checkAppoExistOrNot != null){
	...
	Appointment getAppointment = appointmentRepository.findById(appointmentId).get();
	Appointment appointment = Optional.ofNullable(getAppointment).orElseThrow(ResourceUnAvailableException::new);
	return appointment;
	}
